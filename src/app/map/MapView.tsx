...
export default function MapView() {
    ...
    const [activeCategories, setActiveCategories] = useState<string[]>(["Music", "Tech", "Art", "Food"]);
        ...
        <div className="flex h-[calc(100vh-12rem)] bg-[#F6E8D6]">
                ...
                <div className="flex-1 overflow-y-auto">
                    {loading ? (
                        ...
                        filteredEvents.map((event) => (
                            <div key={event.id} className="p-4 border-b border-[#F6E8D6] hover:bg-[#FFE5D4]/20 cursor-pointer">
                                <div className="flex items-start gap-3">
                                    <div 
                                        className="w-2 h-2 rounded-full mt-2" 
                                        style={{ backgroundColor: categories.find(c => c.name === event.category)?.color || "#FFE5D4" }} 
                                    />
            ...
            <div className="flex-1 relative">
                ...
                <div className="absolute top-4 left-4 z-10 flex gap-2">
                    {categories.map((category) => (
                        <Button
                            ...
                            style={{
                                backgroundColor: activeCategories.includes(category.name) ? category.color : "white"
                            }}
                        >
                            <div 
                ...
                <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
                    <Button variant="outline" size="sm" onClick={handleZoomIn} className="bg-white border-[#F6E8D6]">
                        <Plus className="w-4 h-4" />
                    </Button>
                    <Button variant="outline" size="sm" onClick={handleZoomOut} className="bg-white border-[#F6E8D6]">
                        <Minus className="w-4 h-4" />
                    </Button>
                    <Button variant="outline" size="sm" onClick={handleRecenter} className="bg-white border-[#F6E8D6]">
                        <Crosshair className="w-4 h-4" />
                    </Button>
                    <Button variant="outline" size="sm" onClick={handleFullscreen} className="bg-white border-[#F6E8D6]">
                    ...
                    {isLoaded ? (
                        ...
                        >
                            {!loading && filteredEvents.map((event) => (
                                <Marker
                                    ...
                                    icon={createMarkerIcon(categories.find(c => c.name === event.category)?.color || "#FFE5D4")}
                                />
                            ))}
                                ...
                                >
                                    <div className="p-2 max-w-xs">
                                        ...
                                        <Link href={`/events/${selectedEvent.id}`}>
                                            <Button size="sm" className="text-xs bg-[#FFE5D4] hover:bg-[#F6E8D6] text-gray-900">
                                                View Details
                                            </Button>
                                        </Link>
                                    </div>
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <p className="text-sm text-gray-600">Click and drag to pan • Scroll to zoom • Click markers for details</p>
                            </div>
                        </div>
                    )}
                </div>
